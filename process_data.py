...
# 将原始的train，valid，test构造数据集
def load(sub):
    with open('/mnt/xk/datasets/1-encoder/'+sub+'/src-dfs-tokening-d.txt', 'r') as f:
        dfss = f.readlines()
    f.close()
    
    with open("/mnt/xk/datasets/1-encoder/"+sub+"/src-"+sub+"-d.txt", "r") as f:
        codes = f.readlines()
    f.close()

    #with open('/mnt/xk/datasets/1-encoder/'+sub+'/tgt-test-d.txt','r') as f:
    #    codes = f.readlines()
    #f.close()
    with open('/mnt/xk/datasets/1-encoder/'+sub+'/tgt-'+sub+'-d.txt','r') as f:
    ...
    for i in range(len(codes)):
        ...
        sample['dfs'] = dfss[i].strip()
        #sample['rev'] = reviews[i].strip()
        sample['nl'] = comments[i].strip()
        json_test = json.dumps(sample)
        jsons.append(json_test+"\n")

    with open('n' + sub + '3.json', 'w+', encoding='utf-8') as f:
        f.writelines(jsons)
    f.close()
